...
-   Take a guess.
  >65% of counties republicans won?
...
```{r}
elections |>
  ggplot(aes(repub_pct_20))+
    geom_histogram()
```
> The distribution is left-skewed
Why is it skewed left?
...
ggplot(elections, aes(y = repub_pct_20, x = repub_pct_16)) +
  geom_point(shape = 3)
```
>It changes the shape of the data points
...
```
>color works because they are data points, fill does nothing.
...
> The relationship is strong and mostly linear
...
> Texas deviated from the trend, with a higher percentage in 2020 than would be predicted by 2016. Polarization might be a cause.
...
```
> The median rent is a moderate, negative, possibly linear relationship. It has some outliers with low rent, low percentage points. It is a better predictor than median age, which is a weaker, positve relationship. 
...
ggplot(elections, aes(y = repub_pct_20, x = historical)) +
  geom_point()
```
> Points cannot be distinguished from each other. The points are shoved into small categories.
...
> There is a difference in average support between historically blue, purple, and red counties. Blue had the lowest percentage, red the highest. Even historically blue counties still had a mean above 50% republican vote percentage in 2020. 
...
ggplot(elections, aes(x = repub_pct_20, fill = historical)) +
  geom_density()
```
> The "red" counties are colored blue in this plot, and vice verse, and the colors overlap.
...
```
> It lets you fill the colors you want manually.
...
ggplot(elections, aes(x = repub_pct_20, fill = historical)) +
  ...
  scale_fill_manual(values = c("blue", "purple", "red"))
```
> Adds transparency to see the color overlap.
...
```
> Separates the colors to distinguish the individual density plots.
...
> I like density plots

-   What is one pro of density plots relative to boxplots?
> Density plots show relative distribution more specifically

-   What is one con of density plots relative to boxplots?
> Outliers are shown clearly in boxplots.
...
ggplot(elections, aes(x = historical, fill = winner_20)) +
  geom_bar()
```
> historical is the x axis category and the winner is separated within thise categories
...
> You lose total counts but gain relative proportions

#### Part b

What's your favorite bar plot from part and why?
> I like the side by side plot because I can see total counts and roughly how close the proportions are.
...
ggplot(elections, aes(x = repub_pct_20, fill = state_abbr)) + 
  geom_density(alpha = 0.5)
```
> Theres too many categories to look at.
...
>MS, AL, NM tend to have a lot of variability, HI has very little.

-   What other interesting patterns do you notice?
> Red states don't have as much variabilty as blue or purple states

-   Does this plot prompt any other questions?
> Why do some blue states have a higher pctage than some red states?
...
ggplot(elections_by_state, aes(y = state_abbr, x = repub_pct_20)) + 
