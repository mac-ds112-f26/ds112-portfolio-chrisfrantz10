...
It is stored in the ICA folder in my portfolio
It is a clean visual version of the quarto file, but you can not make edits.
...
this line appears the same. We didn't run any code there is no code chunk
...
It gives the code chunk and the output
...
```{r}
library(tidyverse)
demo <- read.csv("https://mac-stat.github.io/data/high_peaks.csv")
ggplot(demo, aes(x = rating)) +
geom_bar()
```

```{r}
demo <- read.csv("https://mac-stat.github.io/data/high_peaks.csv")
ggplot(demo, aes(x = elevation)) +
geom_density()
```
